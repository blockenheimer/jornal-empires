...
O monitor usa o SRV do domínio principal para descobrir o host **e a porta** atuais (`*.aternos.host:porta`). A Aternos documenta que o domínio é atualizado para o novo IP e porta a cada inicialização e que o DynIP é o endereço direto alternativo.

O bot não presume `25565` e não usa o proxy principal como fallback de porta. Se o DNS normal falhar, ele tenta resolvedores independentes, DNS-over-HTTPS e DNS sobre TCP/53 antes de concluir que o SRV não está disponível.

Para reduzir esse problema, o bot agora consulta o SRV por quatro caminhos: resolver DNS do próprio sistema, Cloudflare DNS-over-HTTPS, Google DNS-over-HTTPS e DNS sobre TCP/53. Os resultados são combinados e deduplicados antes da consulta do Minecraft, sem usar uma porta fixa como fallback.
...
> **Importante:** `PORT` (por exemplo, `13174`) é exclusivamente a porta HTTP do serviço no Render. A porta do Minecraft Aternos **não é presumida como 25565**: ela é descoberta pelo registro SRV do domínio e pode mudar a cada inicialização.



### Aternos v7
A porta do Minecraft não é mais presumida como 25565 e o proxy principal não é usado como fallback. O bot usa o SRV oficial do domínio para descobrir host + porta atuais; adicionamos fallback DNS sobre TCP/53 para ambientes onde consultas DNS normais falham. ATERNOS_ADDRESS pode ser usado apenas como fallback manual no formato host:porta.
