# Bot de Jornal/Folhetim para RP Geopolítico
...
```bash
npm run deploy   # registra o /jornal no Discord (rodar de novo só se mudar o comando)
npm start        # liga o bot
```

Se você definiu `GUILD_ID`, o comando `/jornal` aparece instantaneamente nesse
servidor. Sem `GUILD_ID`, o registro é global e pode levar até 1h para propagar.
...
- **pais**: tem autocomplete — comece a digitar e o Discord sugere a lista das
  nações do RP (lidas de `assets/flags/`, veja a seção 6.1 abaixo).
...
## 6. Calibrando o calendário do RP (`config.json`)
...
### 6.1 Gerenciando as nações/bandeiras do RP
...
utils/paises.js          -> lê assets/flags/ e monta a lista de nações do RP
utils/serverDate.js     -> calcula a "data do servidor" do RP
assets/flags/            -> bandeiras das nações do RP (.png) + nomes.json
assets/fonts/           -> fonte Tinos (compatível com Times New Roman)
config.json              -> configuração do calendário do RP
```
