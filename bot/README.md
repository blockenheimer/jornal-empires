...
When an online Minecraft response is obtained through the SRV endpoint, the monitor now recovers the actual backend address from the successful TCP connection. It first tries reverse DNS for an `*.aternos.host` DynIP name; if that is unavailable, it stores the numeric remote IP with the discovered port. The remembered endpoint is never replaced by the public `*.aternos.me` address.
...
### Descoberta do endpoint temporário

O monitor não faz login, scraping nem chama API privada da Aternos. Ele usa apenas DNS/SRV e o protocolo público do Minecraft. Quando um endpoint SRV responde com `ONLINE`, o bot captura o endereço remoto da conexão TCP. Se esse endereço tiver reverse DNS em `*.aternos.host`, o hostname é guardado; caso contrário, o IP numérico observado é guardado junto com a porta dinâmica. Assim o bot nunca substitui o endereço temporário por `MineEmpiresOf.aternos.me`.

Essa abordagem não promete descobrir o nome textual oficial `*.aternos.host` quando a Aternos não o expõe via DNS público; nesse caso o valor persistido é o IP numérico real observado na conexão.
...
6. guarda o último endpoint temporário observado (`*.aternos.host:porta` quando disponível, ou IP numérico + porta).
...
### Persistência do endpoint temporário

O último endpoint temporário válido é salvo em:

```text
data/aternos-last-endpoint.json
```

O domínio principal `*.aternos.me` nunca é exibido como o IP público do servidor. A porta `25565` em `ATERNOS_WATCHER_PORT` é apenas a porta do endpoint principal/proxy; a porta dinâmica observada vem do SRV.

Quando o servidor está online, o monitor prefere o resultado do endpoint SRV para recuperar o IP remoto da conexão. Quando o servidor fica offline, o último endpoint confirmado permanece na mensagem e no `/ip`.
