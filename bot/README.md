...
O monitor usa o SRV do domínio principal para descobrir o host **e a porta** temporários (`*.aternos.host:porta`). A porta do DynIP muda a cada vez que o servidor liga, então o bot sempre usa a porta que vier no próprio registro SRV — ela nunca é fixa. Quando o servidor fica offline, `/ip` e os avisos automáticos continuam mostrando o último host:porta temporário conhecido; o domínio principal `MineEmpiresOf.aternos.me` nunca é exibido. Se o SRV estiver momentaneamente indisponível, o bot pode testar apenas o último DynIP conhecido; ele não tenta mais `MineEmpiresOf.aternos.me:25565`, evitando o falso positivo do proxy.

`SERVER_STATE_CONFIRM_READINGS` (padrão `2`) define quantas checagens seguidas são necessárias para confirmar mudanças entre ONLINE/OFFLINE. `SERVER_STARTING_CONFIRM_READINGS` (padrão `1`) controla a confirmação do estado de início, permitindo que a mensagem `SERVIDOR INICIANDO` apareça assim que a Aternos informar que está preparando/iniciando. `SERVER_QUERY_ATTEMPTS` e `SERVER_QUERY_RETRY_DELAY_MS` controlam as tentativas de consulta.

O classificador trata respostas de `offline`, `preparing`, `starting` e `stopping` do proxy Aternos como estados intermediários/indisponíveis, e nunca considera `players.max = 0` como um servidor Minecraft real ONLINE. Isso evita a sala de espera como falso positivo.
