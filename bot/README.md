...
O bot resolve `_minecraft._tcp.<domínio>` para descobrir o host/porta atuais (por exemplo, `dhufish.aternos.host:41235` — a porta muda a cada início do servidor) e faz um status ping do Minecraft. O domínio principal usado para a resolução não aparece nas mensagens do Discord.
...
O monitor usa o SRV do domínio principal para descobrir o host **e a porta** temporários (`*.aternos.host:porta`). A porta do DynIP muda a cada vez que o servidor liga, então o bot sempre usa a porta que vier no próprio registro SRV — ela nunca é fixa. Quando o servidor fica offline, `/ip` e os avisos automáticos continuam mostrando o último host:porta temporário conhecido; o domínio principal `MineEmpiresOf.aternos.me` nunca é exibido.

Opcional: `SERVER_STATE_CONFIRM_READINGS` (padrão `2`) define quantas checagens seguidas com o mesmo resultado são necessárias antes de anunciar uma mudança de ONLINE/OFFLINE no Discord — isso evita que uma falha momentânea de rede dispare avisos falsos.
