...
```bash
npm run deploy   # registra os slash commands e limpa versões globais duplicadas quando GUILD_ID estiver definido
npm start        # liga o bot
```

Se você definiu `GUILD_ID`, os comandos aparecem instantaneamente nesse
servidor. O deploy também remove versões globais antigas dos mesmos comandos,
evituando que o Discord mostre, por exemplo, dois `/jornal`. Sem `GUILD_ID`,
o registro é global e pode levar até 1h para propagar.
