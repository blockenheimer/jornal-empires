...
async function generateNewspaperImage(opts) {
  ...
  // ---------- Medidas do texto (com capitular) ----------
  ...
  // A capitular tem a base alinhada com a base da 1ª linha (não desce nas
  // linhas seguintes), então só a 1ª linha precisa abrir espaço pra ela.
  const capLinesCount = 1;
  measure.font = `bold ${capSize}px "${FONT_REGULAR}"`;
  const primeiraLetra = texto.trim().charAt(0);
  const restoTexto = texto.trim().slice(1);
  const capMetrics = measure.measureText(primeiraLetra);
  const capWidth = Math.ceil(capMetrics.width) + 14;
  ...
  const capAscent = capMetrics.actualBoundingBoxAscent;
  const capDescent = capMetrics.actualBoundingBoxDescent;

  measure.font = `${textoFontSize}px "${FONT_REGULAR}"`;
  const textoLines = wrapTextVariable(measure, restoTexto, (lineIdx) =>
    lineIdx < capLinesCount ? CONTENT_WIDTH - capWidth : CONTENT_WIDTH
  );
  const textoH = textoLines.length * textoLineHeight;
  ...
  // ---- Corpo do texto com capitular ----
  ctx.textAlign = 'left';
  ctx.textBaseline = 'alphabetic';

  // capitular: base alinhada com a base da última linha recuada ao lado dela
  ...
  textoLines.forEach((line, idx) => {
    const recuado = idx < capLinesCount;
    const x = recuado ? MARGIN + capWidth : MARGIN;
    const w = recuado ? CONTENT_WIDTH - capWidth : CONTENT_WIDTH;
    const isLast = idx === textoLines.length - 1;
    if (!isLast) {
