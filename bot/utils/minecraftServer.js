...
async function resolveMinecraftEndpoint(domain) {
  ...
  // Java Edition: Aternos normally exposes the dynamic host through SRV.
  try {
    ...
    if (records.length > 0) {
      ...
      if (!publicHost.toLowerCase().endsWith('.aternos.host')) {
        throw new Error(`SRV retornou um host não temporário: ${publicHost}`);
      }
    ...
    if (error?.message?.startsWith('SRV retornou')) throw error;
    // Tente CNAME abaixo. Isso também cobre configurações Aternos que não
    // expõem o DynIP via SRV (por exemplo, fluxos de Bedrock).
  }

  // Fallback: algumas configurações Aternos apontam o domínio para o DynIP
  // usando CNAME em vez de SRV. Nunca exibimos nem usamos o domínio principal
  // como o endereço público; aceitamos somente um alvo *.aternos.host.
  try {
    ...
    // Sem CNAME: o chamador manterá o último DynIP conhecido.
  }
