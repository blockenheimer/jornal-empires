...
const DNS_SERVERS = String(process.env.SERVER_DNS_SERVERS || '1.1.1.1,8.8.8.8')
  ...
  .filter(Boolean);
const DNS_TCP_TIMEOUT_MS = Math.max(1000, Number(process.env.SERVER_DNS_TCP_TIMEOUT_MS || 4000));
...
function sanitizeProbeEndpoint(endpoint, sourceOverride = null) {
  const host = String(endpoint?.publicHost || endpoint?.host || '').trim().replace(/\.$/, '');
  const port = Number(endpoint?.port);
  if (!host || !Number.isInteger(port) || port <= 0 || port > 65535) return null;
  const isMainAternosDomain = host.toLowerCase().endsWith('.aternos.me');
  if (!isTemporaryHost(host) && !isMainAternosDomain) return null;
  return {
    host,
    publicHost: isMainAternosDomain ? null : host,
    port,
    source: sourceOverride || endpoint.source || 'probe',
    internal: isMainAternosDomain || Boolean(endpoint.internal)
  };
}
...
function encodeDnsName(name) {
  const labels = cleanDomain(name).split('.').filter(Boolean);
  const chunks = [Buffer.from([0])];
  const encoded = [];
  for (const label of labels) {
    const value = Buffer.from(label, 'ascii');
    if (value.length > 63) throw new Error(`Label DNS grande demais: ${label}`);
    encoded.push(Buffer.from([value.length]), value);
  }
  encoded.push(Buffer.from([0]));
  return Buffer.concat(encoded.length ? encoded : chunks);
}

function makeDnsQuery(name, type) {
...
function readDnsName(buffer, offset) {
  ...
  while (cursor < buffer.length) {
    if (visited.has(cursor)) throw new Error('Loop de compressão DNS.');
    visited.add(cursor);
    ...
    if ((len & 0xc0) === 0xc0) {
      if (cursor + 1 >= buffer.length) throw new Error('Ponteiro DNS truncado.');
      const pointer = ((len & 0x3f) << 8) | buffer[cursor + 1];
      if (pointer >= buffer.length) throw new Error('Ponteiro DNS inválido.');
    ...
    if (len > 63 || cursor + 1 + len > buffer.length) throw new Error('Label DNS inválido.');
    labels.push(buffer.subarray(cursor + 1, cursor + 1 + len).toString('ascii'));
    cursor += 1 + len;
    if (!jumped) nextOffset = cursor;
  }

  throw new Error('Nome DNS truncado.');
}

function parseDnsResponse(buffer, queryId) {
  if (!Buffer.isBuffer(buffer) || buffer.length < 12) throw new Error('Resposta DNS truncada.');
  const id = buffer.readUInt16BE(0);
  const flags = buffer.readUInt16BE(2);
  const qd = buffer.readUInt16BE(4);
  const an = buffer.readUInt16BE(6);
  if (id !== queryId) throw new Error('ID de consulta DNS não corresponde.');
  if ((flags & 0x8000) === 0) throw new Error('Resposta DNS sem bit de resposta.');
  ...
  for (let i = 0; i < qd; i += 1) {
    ...
    offset = qname.offset + 4;
    if (offset > buffer.length) throw new Error('Pergunta DNS truncada.');
  }

  const answers = [];
  for (let i = 0; i < an; i += 1) {
    const nameInfo = readDnsName(buffer, offset);
    offset = nameInfo.offset;
    if (offset + 10 > buffer.length) throw new Error('RR DNS truncado.');
    const type = buffer.readUInt16BE(offset); offset += 2;
    const cls = buffer.readUInt16BE(offset); offset += 2;
    const ttl = buffer.readUInt32BE(offset); offset += 4;
    const rdlength = buffer.readUInt16BE(offset); offset += 2;
    if (offset + rdlength > buffer.length) throw new Error('RDATA DNS truncado.');
    ...
    if (type === 1 && rdlength === 4) {
      ...
      data = parts.join(':');
    } else if (type === 33 && rdlength >= 7) {
      const priority = rdata.readUInt16BE(0);
      const weight = rdata.readUInt16BE(2);
      const port = rdata.readUInt16BE(4);
      const target = readDnsName(buffer, rdataOffset + 6).name.replace(/\.$/, '');
      data = { priority, weight, port, target };
    }

    answers.push({ name: nameInfo.name, type, class: cls, ttl, data });
    offset += rdlength;
  }
...
function queryDnsOverTcp(name, type, server, timeoutMs = DNS_TCP_TIMEOUT_MS) {
  const { id, packet } = makeDnsQuery(name, type);
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(new Error(`Timeout DNS TCP ${server}.`)), timeoutMs);
    ...
    socket.on('data', (chunk) => {
      ...
      if (buffer.length < frameLen + 2) return;
      const response = buffer.subarray(2, frameLen + 2);
      try { finish(null, parseDnsResponse(response, id)); }
      catch (error) { finish(error); }
    });
  });
}

async function resolveSrvViaDnsTcp(domain, providerIp) {
  const answers = await queryDnsOverTcp(`_minecraft._tcp.${domain}`, 33, providerIp);
  return answers
    .filter((answer) => answer.type === 33 && answer.data && Number.isInteger(answer.data.port))
    .map((answer) => ({
      name: answer.data.target,
      port: answer.data.port,
      priority: answer.data.priority,
      weight: answer.data.weight,
      source: `tcp-${providerIp}`
    }));
}
...
function parseConfiguredAternosAddress(value) {
  const raw = String(value || '').trim();
  if (!raw) return null;
  const match = raw.match(/^\[([^\]]+)\]:(\d{1,5})$/);
  if (match) return { host: match[1], port: Number(match[2]), source: 'configured-address', publicHostDetected: false, internal: true };
  const simple = raw.match(/^(.+):(\d{1,5})$/);
  if (!simple || /^[^:]+$/.test(simple[1]) === false && net.isIP(simple[1]) === 6) return null;
  if (simple && Number(simple[2]) >= 1 && Number(simple[2]) <= 65535) {
    const host = simple[1].trim().replace(/\.$/, '');
    return { host, port: Number(simple[2]), source: 'configured-address', publicHostDetected: false, internal: true };
  }
  return null;
}

async function resolveMinecraftEndpointRecords(domain) {
  const configuredRaw = process.env.ATERNOS_ADDRESS || domain;
  const configured = parseConfiguredAternosAddress(configuredRaw);
  const clean = configured ? cleanDomain(configured.host) : cleanDomain(domain);
  ...
  if (configured) {
    results.push({
      ...
      priority: -1,
      weight: 0,
      publicHost: isTemporaryHost(configured.host) ? configured.host : null
    });
  }
  const errors = [];
  ...
  // Fallback adicional: DNS sobre TCP/53. Alguns ambientes de hospedagem
  // bloqueiam DNS UDP/53 e/ou interceptam o resolvedor local, mas permitem
  // consultas DNS TCP diretas. Isso ainda consulta o SRV oficial da Aternos,
  // portanto a porta descoberta continua sendo a porta publicada pela Aternos.
  const tcpResults = await Promise.allSettled(
    DNS_SERVERS.map(async (server) => ({
      server,
      records: await resolveSrvViaDnsTcp(clean, server)
    }))
  );
  for (const result of tcpResults) {
    if (result.status === 'fulfilled') results.push(...result.value.records);
    else errors.push(`dns-tcp: ${result.reason?.message || result.reason}`);
  }
...
function mergeEndpointCandidate(list, endpoint) {
  const safe = sanitizeProbeEndpoint(endpoint, endpoint?.source || 'srv');
  if (!safe) return;
...
async function getServerStatus({ domain, timeoutMs = DEFAULT_TIMEOUT_MS, lastEndpoint = null }) {
  ...
  // O último DynIP também é testado, mas somente como candidato adicional.
  if (safeLastEndpoint) mergeEndpointCandidate(endpoints, safeLastEndpoint);

  // A porta NÃO é presumida como 25565. Para Aternos, a porta publicada é
  // dinâmica e vem do SRV do domínio (ou de ATERNOS_ADDRESS quando fornecido
  // explicitamente). O último DynIP conhecido é apenas fallback de consulta.
  const candidates = [];
  for (const endpoint of endpoints) candidates.push(endpoint);

  if (!candidates.length) {
    if (DEBUG_STATUS) {
      console.warn(`❌ Nenhum candidato Aternos disponível. SRV: ${resolutionDetails.length ? resolutionDetails.join(' | ') : 'nenhuma resposta'}`);
    }
    return {
      endpoint: safeLastEndpoint,
      online: false,
      state: SERVER_STATE.UNKNOWN,
      stateReason: 'no-probe-target',
      ping: null,
      diagnostics: [],
      resolutionDetails,
      error: resolutionError || new Error('Nenhum método de consulta disponível.')
    };
  }

  const diagnostics = [];
  const queryDeadline = Date.now() + STATUS_CHECK_BUDGET_MS;
  const selectedCandidates = candidates.slice(0, Math.max(MAX_ENDPOINTS, 1));

  if (DEBUG_STATUS) {
    const labels = selectedCandidates.map((item) => `${item.host}:${item.port}/${item.source}`).join(', ');
    console.log(`🧭 Aternos candidatos: ${labels || 'nenhum'}`);
    if (resolutionDetails.length) console.log(`   ↳ DNS/SRV: ${resolutionDetails.join(' | ')}`);
  }

  const results = await Promise.allSettled(
    selectedCandidates.map((candidate) => queryEndpoint(candidate, clean, timeoutMs, queryDeadline))
  );
  ...
  for (let index = 0; index < results.length; index += 1) {
    ...
    if (result.status === 'fulfilled') {
      ...
      if (DEBUG_STATUS) {
        ...
        console.log(`📡 ${candidate.host}:${candidate.port}/${candidate.source} → ${status.state.toUpperCase()} (${status.stateReason || 'sem motivo'})${diag?.connectHost ? ` via ${diag.connectHost}` : ''}`);
      }
    } else {
      const error = result.reason;
      if (Array.isArray(error?.diagnostics)) diagnostics.push(...error.diagnostics);
      if (DEBUG_STATUS) console.log(`   ↳ ${candidate.host}:${candidate.port}/${candidate.source} sem resposta: ${error?.message || error}`);
    }
  }

  const best = combineStatuses(statuses);
  if (best) {
    // Nunca substituímos um DynIP público pelo domínio principal do proxy.
    const displayEndpoint = isValidTemporaryEndpoint(best.endpoint)
      ? best.endpoint
      : safeLastEndpoint;
    ...
    return {
      ...
      internalEndpoint: best.endpoint?.internal ? { host: best.endpoint.host, port: best.endpoint.port, source: best.endpoint.source } : null,
      diagnostics,
      resolutionDetails
    };
  }
  ...
  return {
    ...
    online: false,
    state: SERVER_STATE.UNKNOWN,
    stateReason: resolutionError ? 'query-failed' : 'query-unavailable',
    ping: null,
    diagnostics,
    resolutionDetails,
    error: resolutionError || new Error('Não foi possível confirmar o estado do servidor.')
  };
}
