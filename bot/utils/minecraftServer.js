...
async function resolveSrvViaDoh(domain, provider) {
  ...
  return answers.map((answer) => parseSrvAnswer(answer.data)).filter(Boolean);
}
  ...
  for (const result of dohResults) {
    if (result.status === 'fulfilled') {
      for (const record of result.value.records) {
        results.push({ ...record, source: `doh-${result.value.provider}` });
      }
    } else {
...
async function resolveHostAddresses(host) {
  ...
  for (const provider of DOH_PROVIDERS) {
    const providerResults = await Promise.allSettled(queryTypes.map(async (type) => {
      const query = `${provider.url}?name=${encodeURIComponent(value)}&type=${type}&cd=1&_=${Date.now()}-${Math.random().toString(16).slice(2)}`;
      const data = await httpsJson(query);
    ...
    providerResults.forEach((result, index) => {
      ...
      for (const answer of result.value) {
        if (typeof answer?.data === 'string') add(answer.data, `doh-${provider.name}-${queryTypes[index]}`);
      }
    });
  }
